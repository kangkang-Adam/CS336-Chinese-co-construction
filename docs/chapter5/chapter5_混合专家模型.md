...
- **超额处理**：若某专家被指派的token超过其容量，超出部分**不会执行该专家的FFN**（即被“丢弃”），这些token仅通过残差传递到下一层；因此超额token不会为该专家产生梯度。
...
>Switch Transformer强调稳定性与简化FLOPs，DeepSeekMoE强调精度路径分级、低精度通信和专家更加“专业化”。两者共同构成了现代MoE的两条核心设计思路：
>
>   ① 精度与通信的分级设计，降低训练成本；
>
