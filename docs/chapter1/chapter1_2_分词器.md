...
>最大化语料的似然来学习一组子词概率即选择概率较高的子词，使语料中的字符序列能够被高概率token组合有效表示。换句话说，它的目标是让模型用现有词——数字映射的词表尽可能“解释”自然语言，使模型认为语料中出现的句子更有可能出现。
...
2. **下游使用前的验证与评估**
   ...
   - 如果后续需要扩表如加入新领域术语、专业词或品牌名等，建议优先采用这些方式，而非完全重训tokenizer：**增量训练**、**加入新的merges项**、**清理极低频token**。
